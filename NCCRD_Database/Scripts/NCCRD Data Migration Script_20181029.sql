...
BEGIN
	...
	(
		SELECT
			P.ProjectId,
			ISNULL(LMP.Id, ISNULL(DMP.Id, PRV.Id)) AS [RegionId]
		FROM
			[NCCRD].[dbo].[tb_erm_project_details] PD
		INNER JOIN
			[NCCRD_TEST].[dbo].[Project] P
		...
		LEFT OUTER JOIN
			(
				...
				INNER JOIN
					[VMS_TEST].[dbo].[Regions] REG
					ON REG.Value = LMP.LocalMunicipalityName
				INNER JOIN
					[VMS_TEST].[dbo].[RegionTypes] LT
					ON LT.Id = REG.TypeId
				WHERE
