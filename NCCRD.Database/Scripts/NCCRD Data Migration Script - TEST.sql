...
--Title--
IF (SELECT COUNT(*) FROM [NCCRDv2_TEST].dbo.Title) = 0
BEGIN
	INSERT INTO
		[NCCRDv2_TEST].dbo.Title
		(
			[Value]
		)
	SELECT
...
--Country--
IF (SELECT COUNT(*) FROM [NCCRDv2_TEST].dbo.Country) = 0
BEGIN
	INSERT INTO
		[NCCRDv2_TEST].dbo.Country
		(
			[Value]
		)
	SELECT
...
IF (SELECT COUNT(*) FROM [NCCRDv2_TEST].[dbo].[UserRoles]) = 0
BEGIN
	INSERT INTO
		[NCCRDv2_TEST].[dbo].[UserRoles]
		(
			RoleName
		)	
	SELECT
